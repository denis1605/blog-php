...
switch ($act) {
	...
	while ($row =  $sel->fetch_assoc()) {
		$row['date'] = date('Y-n-d H:i:s', $row['date']);
	...
	$ENTRY['date'] = date('Y-n-d H:i:s', $ENTRY['date']);
	...
	case 'do-new-entry':
	if (!IS_ADMIN) die ("You must be admin!");
	...
	if (!IS_ADMIN) die ("You must be admin!");
	$id = intval($_GET['id']);
	$mysqli->query("DELETE FROM comment WHERE id = $id") or die("Cannot delete commet");
